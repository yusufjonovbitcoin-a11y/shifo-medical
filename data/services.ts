...
export interface Service {
  ...
  description: string; // Fallback (Russian)
  price: string; // Fallback (Russian)
  gradient: string;
  servicesList?: string[];
...
На заключительном этапе операционная рана дренируется для отведения тканевой жидкости (обычно дренаж можно убрать уже через несколько дней). После операции пациентка остается под наблюдением, пока не придет в себя после наркоза и все жизненные показатели не стабилизируются. Затем большинство пациенток покидают клинику уже через 1-2 дня. На протяжении еще 7-14 дней пациентка наблюдается у врача, а впоследствии, если необходимо, проводят операцию по реконструкции груди. Некоторые пациентки предпочитают отсрочить реконструкцию – в таких случаях на временной или постоянной основе могут применяться грудные протезы.`,
  ...
  {
    icon: Activity,
    serviceKey: "electrocardiography",
