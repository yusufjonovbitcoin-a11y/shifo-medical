...
// Static fayllarni serve qilish (test.html va boshqa frontend fayllar uchun)
app.use(express.static(join(__dirname, '..')));

// Root GET route - test.html'ni serve qilish
app.get('/', (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      usage: 'test.html faylini brauzerda oching yoki POST /ai-chat endpoint\'idan foydalaning'
    });
  }
});
...
app.listen(PORT, () => {
  ...
  if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === 'your-openai-api-key-here') {
    ...
    console.log('⚠️  Iltimos, backend/.env fayliga haqiqiy OpenAI API key kiriting!');
